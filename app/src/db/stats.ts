...
export function lastSets(exercise_id: number | string) {
  ...
  if (!row) return [];
  return db.prepare('SELECT set_type, weight, reps, duration_s, distance_m, incline, speed, avg_hr, rpe FROM sets WHERE workout_exercise_id = ? AND completed = 1 ORDER BY position').all(row.id);
}
